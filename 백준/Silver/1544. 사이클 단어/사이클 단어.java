...
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        ArrayList<String> arr = new ArrayList<String>();    // 새로운 글자가 입력되면 저장할 리스트 생성
        for(int i = 0; i < n; i++) {
            String s = br.readLine();
            boolean isequal = false; 
            
            for(int j = 0; j < arr.size(); j++) {
                if(arr.get(j).length() == s.length()) {    // 저장되어 있는 단어들 중 현재 입력단어와 글자 수가 똑같은것들만 비교 
                    if(compare(arr.get(j), s)) {
                        isequal = true;
                        break;
                    }
                }
        ...
        System.out.println(arr.size());    // 리스트의 사이즈가 문자들의 종류이므로 출력시켜줌
    }
